...
#echo "${COMMAND}"
echo "${STAGE_COMMAND}" | envsubst >> /tmp/github_action/${uuid}/_remotescript.sh
echo 'echo 0 > '/tmp/github_action/${uuid}/_remotescript_returncode.txt >> /tmp/github_action/${uuid}/${stage}_remotescript.sh
chmod +x /tmp/github_action/${uuid}/${stage}_remotescript.sh
...
scp -P 2222 -o 'StrictHostKeyChecking=no' -o 'IdentitiesOnly=yes' -i /tmp/git_action_ssh_id_rsa -F /dev/null /tmp/github_action/${uuid}/_remotescript.sh daqian@127.0.0.1:/tmp/github_action/${uuid}/${stage}-github-aciton.sh
